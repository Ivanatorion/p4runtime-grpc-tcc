...
public class ReactiveForwarding {
    ...
    private void requestIntercepts() {
        ...
        log.warn(String.valueOf(selector));
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            log.info(String.valueOf(pkt));
            // Are we on an edge switch that our destination is on? If so,
            // simply forward out to the destination and bail.
            DeviceId dst_location = DeviceId.deviceId("device:bmv2:s1");
            DeviceId dst_location2 = DeviceId.deviceId("device:bmv2:s2");
            DeviceId dst_location3 = DeviceId.deviceId("device:bmv2:s3");
            PortNumber portNumber = PortNumber.portNumber(3);
            if (pkt.receivedFrom().deviceId().equals(dst_location) || pkt.receivedFrom().deviceId().equals(dst_location2) || pkt.receivedFrom().deviceId().equals(dst_location3)) {
                log.info("device id ok");
                log.info(String.valueOf(context.inPacket().receivedFrom().port()));
