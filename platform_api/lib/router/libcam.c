...
uint32_t send_frame_ID = 2;
CAM_CONTEXT CAM_CONTEXT_send_frame;


uint32_t forward_table_ID = 1;
CAM_CONTEXT CAM_CONTEXT_forward_table;


uint32_t ipv4_nhop_ID = 0;
CAM_CONTEXT CAM_CONTEXT_ipv4_nhop;
...
void init_send_frame() {
    CAM_CONTEXT* cx = &CAM_CONTEXT_send_frame;
...
void init_forward_table() {
    CAM_CONTEXT* cx = &CAM_CONTEXT_forward_table;
    uint32_t size = CAM_Init_GetAddrSize();
    // TODO: set baseAddr to the base address of the table
...
void init_ipv4_nhop() {
    CAM_CONTEXT* cx = &CAM_CONTEXT_ipv4_nhop;
    uint32_t size = CAM_Init_GetAddrSize();
    // TODO: set baseAddr to the base address of the table
...
int cam_read_entry(uint32_t tableID, char* key, char* value, char* found) {
    ...
    if (tableID == send_frame_ID) {
        init_send_frame();
        cx = &CAM_CONTEXT_send_frame;
    }

    if (tableID == forward_table_ID) {
        init_forward_table();
        cx = &CAM_CONTEXT_forward_table;
    }

    if (tableID == ipv4_nhop_ID) {
        init_ipv4_nhop();
        cx = &CAM_CONTEXT_ipv4_nhop;
    }
...
int cam_add_entry(uint32_t tableID, const char* key, const char* value) {
    ...
    if (tableID == send_frame_ID) {
        init_send_frame();
        return CAM_Mgt_InsertEntry(&CAM_CONTEXT_send_frame, key, value, 0);
    }

    if (tableID == forward_table_ID) {
        init_forward_table();
        return CAM_Mgt_InsertEntry(&CAM_CONTEXT_forward_table, key, value, 0);
    }

    if (tableID == ipv4_nhop_ID) {
        init_ipv4_nhop();
        return CAM_Mgt_InsertEntry(&CAM_CONTEXT_ipv4_nhop, key, value, 0);
    }
...
int cam_delete_entry(uint32_t tableID, const char* key) {
    ...
    if (tableID == send_frame_ID) {
        init_send_frame();
        return CAM_Mgt_RemoveEntry (&CAM_CONTEXT_send_frame, key);
    }

    if (tableID == forward_table_ID) {
        init_forward_table();
        return CAM_Mgt_RemoveEntry (&CAM_CONTEXT_forward_table, key);
    }

    if (tableID == ipv4_nhop_ID) {
        init_ipv4_nhop();
        return CAM_Mgt_RemoveEntry (&CAM_CONTEXT_ipv4_nhop, key);
    }
...
uint32_t cam_get_size(uint32_t tableID) {
    ...
    if (tableID == send_frame_ID) {
        init_send_frame();
        return CAM_Mgt_GetSize(&CAM_CONTEXT_send_frame);
    }

    if (tableID == forward_table_ID) {
        init_forward_table();
        return CAM_Mgt_GetSize(&CAM_CONTEXT_forward_table);
    }

    if (tableID == ipv4_nhop_ID) {
        init_ipv4_nhop();
        return CAM_Mgt_GetSize(&CAM_CONTEXT_ipv4_nhop);
    }
