...
CAM_CONTEXT CAM_CONTEXT_forward;
...
void init_forward() {
    CAM_CONTEXT* cx = &CAM_CONTEXT_forward;
...
int cam_read_entry(uint32_t tableID, char* key, char* value, char* found) {
    ...
    if (tableID == forward_ID) {
        init_forward();
        cx = &CAM_CONTEXT_forward;
    }
...
int cam_add_entry(uint32_t tableID, const char* key, const char* value) {
    ...
    if (tableID == forward_ID) {
        init_forward();
        return CAM_Mgt_InsertEntry(&CAM_CONTEXT_forward, key, value, 0);
    }
...
int cam_delete_entry(uint32_t tableID, const char* key) {
    ...
    if (tableID == forward_ID) {
        init_forward();
        return CAM_Mgt_RemoveEntry (&CAM_CONTEXT_forward, key);
    }
...
uint32_t cam_get_size(uint32_t tableID) {
    ...
    if (tableID == forward_ID) {
        init_forward();
        return CAM_Mgt_GetSize(&CAM_CONTEXT_forward);
    }
