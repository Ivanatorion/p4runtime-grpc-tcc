...
void init_dmac() {
    CAM_CONTEXT* cx = &CAM_CONTEXT_dmac;
...
int cam_add_entry(uint32_t tableID, const char* key, const char* value) {
    ...
    if (tableID == dmac_ID) {
        init_dmac();
        return CAM_Mgt_InsertEntry(&CAM_CONTEXT_dmac, key, value, 0);
    }
...
int cam_delete_entry(uint32_t tableID, const char* key) {
    ...
    if (tableID == dmac_ID) {
        init_dmac();
        return CAM_Mgt_RemoveEntry (&CAM_CONTEXT_dmac, key);
    }
...
uint32_t cam_get_size(uint32_t tableID) {
    ...
    if (tableID == dmac_ID) {
        init_dmac();
        return CAM_Mgt_GetSize(&CAM_CONTEXT_dmac);
    }
